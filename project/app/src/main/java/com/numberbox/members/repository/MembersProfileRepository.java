...
public interface MembersProfileRepository extends JpaRepository<MembersProfile, Long> {
	...
	public int changeProfileImg(@Param("uuid") UUID uuid, @Param("profileImgPath") String profileImgPath,
			@Param("profileImgName") String profileImgName);
	...
	@Query(value = "UPDATE MembersProfile m " + "SET m.profileType=6 where m.userUniqId IN "
	...
	@Transactional
	@Modifying // select 문이 아님을 나타낸다(로컬 mysql에서는 안돌아감, 서버 mariaDB에서는 돌아감)
	@Query(value = "UPDATE MembersProfile m " + "SET m.profileType=5 where m.userUniqId IN "
			+ "(SELECT A.userUniqId FROM MembersProfile A INNER JOIN MembersPrivate B on A.userUniqId=B.userUniqId "
			+ "WHERE  SUBSTRING(B.birth, 1, 2)>:birthYear and SUBSTRING(B.birth, 1, 2)<=:nowYear)", nativeQuery = false)
	public int updateTeenagersProfileTypeToStudent(@Param("birthYear") String birthYear,
			@Param("nowYear") String nowYear);
