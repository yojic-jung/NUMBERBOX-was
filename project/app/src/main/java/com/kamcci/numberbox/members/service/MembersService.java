package com.kamcci.numberbox.members.service;

import com.kamcci.modules.auth.control.util.AuthPasswordEncoder;
import com.kamcci.numberbox.common.util.ClientConnect;
import com.kamcci.numberbox.common.util.CommonUtil;
import com.kamcci.numberbox.common.util.CustomTenFieldDto;
import com.kamcci.numberbox.jwt.service.RefreshTokenInfoService;
import com.kamcci.numberbox.mathinfo.repository.MathContentsRepository;
import com.kamcci.numberbox.members.dto.*;
import com.kamcci.numberbox.members.entity.*;
import com.kamcci.numberbox.members.repository.*;
...
import com.kamcci.numberbox.members.restapi.dto.request.PasswordRequest;
...
public class MembersService {
    ...
    @Transactional
    public Map<String, Object> registerProfileImg(MembersProfileDto membersProfileDto, String path) throws IllegalStateException, IOException {
        ...
        if(membersProfileDto.getProfileImgFile() != null && !membersProfileDto.getProfileImgFile().isEmpty()) {
            ...
            String fileName = currentTime1 + "_" + randomValue1 + "_" + membersProfileDto.getProfileImgFile().getOriginalFilename();
    ...
    public Map<String, Object> findPassword(HttpServletRequest request, String email) throws MessagingException {
        HashMap<String, Object> map = new HashMap<>();
        ...
        if(members != null) {
            ...
            String contents = "<div style='width:500px;height:600px; font-family:\"Malgun Gothic\";background: rgb(226, 224, 224);padding:30px 100px;'><div style='width:350px; margin:150px auto;line-height:180%; padding:20px;background:white;'><div style='color:#3e6599;font-size:25px;'>비밀번호 안내</div><br/><div style='font-size:15px;'>안녕하세요. 회원님의 요청으로 발급해드리는 <br/>임시 비밀번호는 <span style='font-weight:bold;'>" + randPasswrod + "</span> 입니다.</div><br/><div style='font-weight:bold;background:rgb(236, 250, 106);font-size:13px; padding:10px;word-break:keep-all;'>임시 비밀번호는 오전 06시까지 유효하니 로그인 후<br/>임시 비밀번호를 변경하여 주시기 바랍니다.</div><br/><div style='text-align:center;'><br/><a href='https://nsoohak.com/login' style='text-decoration:none'><span style='text-decoration:none;font-size:18px;border:none; border-radius:14px; padding:10px; background:#3e6599; color:white;cursor:pointer;font-weight:bold'>N명의수학 로그인하기</span></a></div></div></div>";
    ...
    public Map<String, Object> changePassword(PasswordRequest passwordRequest) {
        ...
        Members members = StaticSecurityUtil.getMembers();
        Members corfirmMembers = membersRepository.findByEmail(members.getEmail());
        boolean isCertified = authPasswordEncoder.matches(passwordRequest.getOldPassword(), corfirmMembers.getPassword());
        if(isCertified) {
    ...
    public Map<String, Object> changePhoneNumber(MembersRequest memberDto) {
        ...
        Members corfirmMembers = membersRepository.findByEmail(members.getEmail());
        MembersPrivate membersPrivate = membersPrivateRepository.findByUserUniqId(corfirmMembers.getUserUniqId());
        boolean isUserCertified = membersPrivate.getUserName().equals(memberDto.getUserName());
    ...
    public Map<String, Object> myAccountDrop(MembersRequest memberDto) {
        ...
        MembersPrivate membersPrivate = membersPrivateRepository.findByUserUniqId(corfirmMembers.getUserUniqId());
        if(!membersPrivate.getUserName().equals(memberDto.getUserName()) || !membersPrivate.getPhoneNumber().equals(memberDto.getPhoneNumber()) || !membersPrivate.getBirth().equals(memberDto.getBirth())) {
            map.put("existMsg", true);
    ...
    public Map<String, Object> createEmailIdCode(String email) {
        ...
        if(isSuccess) {
            // 메일 전송
            String contents = "<div>안녕하세요. N명의수학입니다.<br/> 요청하신 회원가입 이메일 인증코드는 아래와 같습니다.</div> <div style='margin:\"10px 0\"font-family:\"Malgun Gothic\";font-size:\"20px\"; '>" + idCode + "</div>위 인증코드는 3분간 유효합니다.";
            try {
