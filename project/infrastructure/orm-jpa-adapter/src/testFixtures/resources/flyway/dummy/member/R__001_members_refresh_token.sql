DELETE
FROM `numberbox_tc`.`member_refresh_token`;
--
INSERT INTO `numberbox_tc`.`member_refresh_token` (`id`, `token`, `member_id`, `sys_create_time`, `sys_update_time`)
VALUES ('1',
        'eyJhbGciOiJIUzI1NiJ9.eyJuc29vaGFrLmNvbSI6dHJ1ZSwiaXNzIjoibnNvb2hhayIsInN1YiI6Im5zb29oYWtSZWZyZXNoVG9rZW4iLCJhdWQiOiJ1c2VyIiwiZXhwIjoxNzAxMTU1NjI3LCJpYXQiOjE2OTg1NjM2Mjd9.IRiJaK2jH-3DskfW4N2Rhm9eVzhB9Mswp8-JlfDN-Ws',
        UNHEX('10ED5466CDA8EA4D9BC7037CB86FDB20'), '2023-10-29 16:13:47', '2024-09-20 22:18:26');
