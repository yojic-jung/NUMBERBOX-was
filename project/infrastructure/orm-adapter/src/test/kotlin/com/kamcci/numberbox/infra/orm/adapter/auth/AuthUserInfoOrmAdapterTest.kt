package com.kamcci.numberbox.infra.orm.adapter.auth
...
@TcDBJpaTest
class AuthUserInfoOrmAdapterTest(
    @Autowired
    private val authUserInfoOrmAdapter: AuthUserInfoOrmAdapter
) {
    @Test
    fun `멤버엔티티 조회 - 성공`() {
        // given
        ...
        // when
        val authUser = authUserInfoOrmAdapter.loadUserByUsername(email)
    ...
    fun `멤버 엔티티 리프시 조회 - 성공`() {
        ...
        // when
        val uuid = authUserInfoOrmAdapter.loadUserIdByRefreshToken(token)
