...
class MemberProfileModifyOrmAdapterTest(
    ...
    @Autowired
    private val memberProfileModifyOrmAdapter: MemberProfileModifyOrmAdapter
) {
    @Test
    fun `프로필 타입 변경`() {
        // given
        ...
        // when
        memberProfileModifyOrmAdapter.updateProfileTypeByMemberId(memberId, profileType)
        entityManager.flush()
        entityManager.clear()
    ...
    fun `프로필 이미지 변경`() {
        ...
        // when
        memberProfileModifyOrmAdapter.updateImgByMemberId(profileImgInfo)
        entityManager.flush()
        entityManager.clear()
    ...
    fun `프로필 닉네임 변경`() {
        ...
        // when
        memberProfileModifyOrmAdapter.updateNicknameByMemberId(memberId, nickname)
        entityManager.flush()
        entityManager.clear()
