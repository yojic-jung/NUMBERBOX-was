...
class HwpConvertEventKafkaAdapter(
    ...
    override fun requestHwp(eventDto: JsonToHwpRequestEvent) {
        try {
            kafkaJsonTemplate.send("numberbox.jsonToHwp.request", eventDto.id.toString(), eventDto)
                .get(9, TimeUnit.SECONDS)
        } catch (e: TimeoutException) {
            log.error("Kafka send timeout", e)
            throw BusinessSeverException("kafka 예외 발생")
    ...
    override fun requestHtml(eventDto: HwpToHtmlRequestEvent) {
        try {
            kafkaHwpTemplate.send("numberbox.hwpToHtml.request", eventDto.id.toString(), eventDto)
                .get(9, TimeUnit.SECONDS)
        } catch (e: TimeoutException) {
            log.error("Kafka send timeout", e)
            throw BusinessSeverException("kafka 예외 발생")
