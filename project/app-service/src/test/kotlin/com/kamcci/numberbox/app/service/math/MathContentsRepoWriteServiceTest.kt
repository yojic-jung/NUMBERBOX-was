...
class MathContentsRepoWriteServiceTest {
    private val mathConRepoReadCase: MathContentsRepoReadCase = mock()
    private val mathConRepoModifyOrmPort: MathContentsRepoWriteOrmPort = mock()
    ...
    fun `문제 저장소 저장 - 성공`() {
        // given
        val modifyDto = getMathContentsRepoModifyDto()
        Mockito.`when`(mathConRepoReadCase.existByContentsIdAndMemberId(modifyDto.contentsId, modifyDto.memberId))
            .thenReturn(false)
