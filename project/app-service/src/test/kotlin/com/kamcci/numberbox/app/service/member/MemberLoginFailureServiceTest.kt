...
import com.kamcci.numberbox.app.service.member.MemberLoginFailureService.Companion.NOT_EXIST_USER
...
import com.kamcci.numberbox.app.service.mock.usecase.member.MockMemberReadCase.Companion.NONE_FAIL_DISABLE_MEMBER
...
class MemberLoginFailureServiceTest {
    companion object {
        const val SUCCESS_EMAIL = "test@test.com"
    ...
    fun `실패 카운트 초과시 계정 비활성화 - 성공`() {
        // given
        val email = SUCCESS_EMAIL
        val memberReadCase = MockMemberReadCase()
        memberReadCase.failCnt = DISABLE_COUNT
    ...
    fun `계정 비활성(실패 카운트 미존재) - 실패`() {
        ...
        assertThat(exception.msg).isEqualTo(NOT_EXIST_USER)
    }
    ...
    fun `게정 장금 활성화(마지막 실패 시간 미존재) - 실패`() {
        ...
        val email = NONE_FAIL_DISABLE_MEMBER
