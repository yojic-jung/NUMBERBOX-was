...
public interface AccessLogInfoRepository extends JpaRepository <AccessLogInfo, Integer> {
	...
	@Query(value ="SELECT "
			+ " COUNT(DISTINCT user_uniq_id, date_format(login_time, '%Y-%m-%d')) "
			+ " FROM access_log_info "
			+ " WHERE login_time>= :minLoginTime and login_time<= :maxLoginTime"
	...
	public int countDistinctUserUniqIdByLoginTimeBetweenAndUserUniqIdNotIn(@Param("minLoginTime") LocalDateTime minLoginTime, @Param("maxLoginTime") LocalDateTime maxLoginTime);
