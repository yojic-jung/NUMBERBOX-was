...
public class CustomSecurityUser extends User {
	...
	public CustomSecurityUser(Members members) {
		super(members.getEmail(), members.getPassword(), enableCheck(members.getRole()), true, true, true,
				makeGrantedeAuth(members.getRole()));
	...
	private static List<GrantedAuthority> makeGrantedeAuth(List<MembersRole> roles) {
		...
		roles.forEach(membersRole -> list.add(new SimpleGrantedAuthority(ROLE_PREFIX + membersRole.getRoleName())));
		return list;
	}
