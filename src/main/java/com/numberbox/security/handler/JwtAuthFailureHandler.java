...
import com.numberbox.security.exception.RefreshTokenExpirationException;
import com.numberbox.security.exception.RefreshTokenNotMachingException;
import com.numberbox.security.exception.RefreshTokenNullException;
...
public class JwtAuthFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletResponse response, Exception exception) {
        if (exception instanceof RefreshTokenNullException) {
            SecurityUtil.respondUnAuthroized(response, "다시 로그인 해주시기 바랍니다.");
            ...
            SecurityUtil.respondUnAuthroized(response, "로그인 유효기간이 종료되었습니다.\n다시 로그인 해주시기 바랍니다.");
            ...
            eventPublish(refreshToken);
            SecurityUtil.respondUnAuthroized(response, "비정상적인 인증 요청입니다.\n다시 로그인 해주시기 바랍니다.");
        } else if (exception instanceof JwtInvalidException) {
            SecurityUtil.respondUnAuthroized(response, "로그인 유효기간이 종료되었습니다.\n다시 로그인 해주시기 바랍니다.");
        } else {
            // 서버 에러
            SecurityUtil.respondInternalServerError(response, "로그인 인증 과정에 실패 하였습니다.");
        }
